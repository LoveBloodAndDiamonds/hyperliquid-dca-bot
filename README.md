...
```cron
0 * * * *   cd /root/hyperliquid-dca-bot && uv run -m app --coin=BTC --amount=0.001 >> logs/cron.log 2>&1
0 */4 * * * cd /root/hyperliquid-dca-bot && uv run -m app --coin=ETH --notional=25  >> logs/cron.log 2>&1
```
